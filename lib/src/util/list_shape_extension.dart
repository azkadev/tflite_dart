extension ListShape on List {
  ...
  List reshape<T>(List<int> shape) {
    ...
    for (var i = dims - 1; i > 0; i--) {
      var temp = [];
      for (var start = 0; start + shape[i] <= reshapedList.length; start += shape[i]) {
    ...
    List<List<List<List<T>>>> reshapedList = List.generate(
      ...
      (i) => List.generate(
        ...
        (j) => List.generate(
          ...
          (k) => List.generate(
            shape[3],
            (l) => flatList[i * shape[1] * shape[2] * shape[3] + j * shape[2] * shape[3] + k * shape[3] + l],
          ),
        ),
      ),
    );
      ...
      (i) => List.generate(
        ...
        (j) => List.generate(
          ...
          (k) => List.generate(
            ...
            (l) => List.generate(
              shape[4],
              (m) => flatList[i * shape[1] * shape[2] * shape[3] * shape[4] + j * shape[2] * shape[3] * shape[4] + k * shape[3] * shape[4] + l * shape[4] + m],
            ),
          ),
        ),
      ),
